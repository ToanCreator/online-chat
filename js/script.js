...
function isValidFullName(name) {
    ...
    return /^[a-zA-Z0-9\sÀÁẠẢÃĂẰẮẶẲẴÂẦẤẬẨẪÈÉẸẺẼÊỀẾỆỂỄÌÍỊỈĨÒÓỌỎÕÔỒỐỘỔỖƠỜỚỢỞỠÙÚỤỦŨƯỪỨỰỬỮỲÝỴỶỸàáạảãăằắặẳẵâầấậẩẫèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹĐđ]{1,20}$/.test(name);
}
...
async function sendMessage(text) {
    ...
    try {
        ...
        const messageData = {
            ...
            senderProfileImage: currentUser.profileImage || "https://static.wikia.nocookie.net/roblox-blox-fruits/images/f/f6/Mochid_v2_Icon.png/revision/latest/scale-to-width-down/250?cb=20240324103138",
...
async function trollUser(userId) {
    ...
    try {
        ...
        await sendSystemMessage(activeGroupId, `Admin đã trôn trôn người dùng ${userSnap.data().name} (ID: ${userId}).`);
